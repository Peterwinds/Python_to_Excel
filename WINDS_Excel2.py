...
#         Allocate transpiration between layers based on depth of roots and actual depletion
            ...
            for k in range(M.Num_layers + 1, Equil_Max, -1): # Equil_Max is the upper layer number of the water table. 
                ...
                if k == M.Num_layers + 1:
                    M.WC[j][k] = M.Active_WC[k] + (Irrigation_surface - M.E_wet[j] - M.ET_trans[j] * M.Act_frac[j][k] + M.wet_Rain_Infilt[j]) / M.Fw[k]  / 1000 / M.dz[k]
                else:
                    M.WC[j][k] = M.Active_WC[k] + (Irrigation_drip_layer[k]+M.Infilt[j][k + 1] - M.ET_trans[j] / 1000 * M.Act_frac[j][k]) / M.dz[k] / M.Fw[k]
                ...
                #Loop 2, if soil is not restricted to field capacity
                ...
                elif M.WC[j][k] > M.FC[k]:
                    if M.WC[j][k] >= M.ts[k]:
                        M.Infilt[j][k] = M.Ksat[k]
                        Original_WC_value = M.WC[j][k]
                        M.WC[j][k] = M.WC[j][k] - M.Infilt[j][k] / M.dz[k] / M.Fw[k]
                        if M.WC[j][k] < M.FC[k]:
                            M.WC[j][k] = M.FC[k]
                            M.Infilt[j][k] = (Original_WC_value - M.FC[k])*M.dz[k]*M.Fw[k]
                    else:
                        M.Infilt[j][k] = (M.WC[j][k] - M.FC[k])*M.dz[k]*M.Fw[k]
                        M.WC[j][k] = M.FC[k]
                    ...
                    Keff[k] = (M.dz[k] + M.dz[k - 1]) / (M.dz[k] / kh[k] + M.dz[k - 1] / kh[k - 1])
