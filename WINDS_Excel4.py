...
import WINDSfunctionsandclasses_Excel  as wmf
...
for i in range(0, Num_plantings): #this loop cycles through all of the plantings
    if (P.RunPlanting[i] == 1): #checks to see whether a planting is active and should be analyzed
        ...
        M.Create_output_layer_arrays(int((M.NumDaysInc + 1) * (M.Num_layers + 1)), M.NumDays, M.Num_layers)
...
#         Allocate transpiration between layers based on depth of roots and actual depletion
            ...
            Depthcalc = 0
            if M.type_irrigation == "combination":
               
                if M.FW_Type[j] == "drip":
                        Irrigation_surface = 0
                        Irrigation_drip_layer[M.Num_layers] = M.Irrigation[j] * Irrigation_partition_one
                        Irrigation_drip_layer[M.Num_layers - 1] = M.Irrigation[j] * Irrigation_partition_two
                        Irrigation_drip_layer[M.Num_layers - 2] = M.Irrigation[j] * Irrigation_partition_three
                        Irrigation_drip_layer[M.Num_layers - 3] = M.Irrigation[j] * Irrigation_partition_four
                        M.Few[j] = 0.5
                elif FW_Type[j] == "furrow":
                        Irrigation_surface = M.Irrigation[j]
                        Irrigation_drip_layer[M.Num_layers] = 0
                        Irrigation_drip_layer[M.Num_layers - 1] = 0
                        Irrigation_drip_layer[M.Num_layers - 2] = 0
                        Irrigation_drip_layer[M.Num_layers - 3] = 0
                        M.Few[j] = 1
                elif FW_Type[j] == "sprinkler":
                        Irrigation_surface = M.Irrigation[j]
                        Irrigation_drip_layer[M.Num_layers] = 0
                        Irrigation_drip_layer[M.Num_layers - 1] = 0
                        Irrigation_drip_layer[M.Num_layers - 2] = 0
                ...
                Irrigation_drip_layer[M.Num_layers - 3] = 0
                M.Few[j] = 1
            elif M.type_irrigation == "sprinkler":
                Irrigation_surface = M.Irrigation[j]
                Irrigation_drip_layer[M.Num_layers] = 0
                Irrigation_drip_layer[M.Num_layers - 1] = 0
                Irrigation_drip_layer[M.Num_layers - 2] = 0
            ...
            for k in range(M.Num_layers + 1, Equil_Max, -1): # Equil_Max is the upper layer number of the water table. 
                WaterContent[0][k] = M.WC[j - 1][k] # M.Active is the water content ###Probably not needed for irrigation. If not, replace it with M.WC
                
            for jj in range(1,11):
                for k in range(M.Num_layers + 1, Equil_Max, -1): # Equil_Max is the upper layer number of the water table. 
                
                    if k == M.Num_layers + 1:
                        Water_content[jj][k] = Water_content[jj - 1][k] + 0.1*((Irrigation_surface - M.E_wet[j] - M.ET_trans[j] * M.Act_frac[j][k] + M.wet_Rain_Infilt[j]) / M.Fw[k]  / 1000 / M.dz[k])
                    else:
                        Water_content[jj][k] = Water_content[jj - 1][k] + Infiltration[jj][k + 1] + 0.1*((Irrigation_drip_layer[k] - M.ET_trans[j] / 1000 * M.Act_frac[j][k]) / M.dz[k] / M.Fw[k])
                    
                #Loop 1, if soil is restricted to field capacity
                
                
                    te1 = wmf.Calculate_te(Water_content[jj - 1][k], M.tr[k], M.ts[k])
                    te2 = wmf.Calculate_te(Water_content[jj - 1][k - 1], M.tr[k - 1], M.ts[k - 1])
                    kh[k] = wmf.Calculate_k(te1, M.mv[k], M.Ko[k], M.Lv[k])
                    kh[k - 1] = wmf.Calculate_k(te2, M.mv[k - 1], M.Ko[k - 1], M.Lv[k - 1])
                    Keff = (M.dz[k] + M.dz[k - 1]) / (M.dz[k] / kh[k] + M.dz[k - 1] / kh[k - 1])
                    head1 = -((te1 ** (-1 / M.mv[k]) - 1) ** (1 / M.nv[k])) / M.av[k] / 100
                    head2 = -((te2 ** (-1 / M.mv[k - 1]) - 1) ** (1 / M.nv[k - 1])) / M.av[k - 1] / 100
                    Infiltration[jj][k] = 0.1*(head1 - head2 + (M.Zave[k] - M.Zave[k - 1])) / (M.Zave[k] - M.Zave[k - 1]) * Keff * M.Fw[k]
                    Water_content[jj][k] = M.WC[j][k] - M.Infilt[j][k] / M.dz[k] / M.Fw[k]
            ...
            if int(M.Nitrogen_simulation) == 1:
        ...
        temp_output['Days_after_planting']=np.linspace(1,1,M.NumDaysInc-2)
        temp_output['DOY']=M.DOY[1:M.NumDaysInc - 1]
        temp_output['pid']=M.PlantingIDO[1:M.NumDaysInc - 1]
        temp_output['plantname']=M.PlantingNameO[1:M.NumDaysInc - 1]
        temp_output['NDVI'] = M.NDVI[1:M.NumDaysInc - 1]
        temp_output['NDVI_regression'] = M.NDVI_regression[1:M.NumDaysInc - 1]
        temp_output['Kcb_NDVI'] = M.Kcb_NDVI[1:M.NumDaysInc - 1]
        temp_output['Kcb_calculated'] = M.Kcb_calculated[1:M.NumDaysInc - 1]
        temp_output['Kcb_used'] = M.Kcb[1:M.NumDaysInc - 1]
        temp_output['one_minus_c'] = M.one_minus_c[1:M.NumDaysInc - 1]
        temp_output['Crop_height'] = M.Crop_height[1:M.NumDaysInc - 1]
        temp_output['Root_depth'] = M.Root[1:M.NumDaysInc - 1]
        temp_output['Ke'] = M.Ke[1:M.NumDaysInc - 1]
        temp_output['Evap'] = M.E[1:M.NumDaysInc - 1]
        temp_output['Transpiration'] = M.ET_trans[1:M.NumDaysInc - 1]
        temp_output['ET'] = M.ET[1:M.NumDaysInc - 1]
        temp_output['Pottranspiration'] = M.ET_pot[1:M.NumDaysInc - 1]
        temp_output['CumulativePotentialET'] = M.ETcum[1:M.NumDaysInc - 1]
        temp_output['CumulativeactualET'] = M.ETcum[1:M.NumDaysInc - 1]
        temp_output['ReferenceET'] = M.ET_0[1:M.NumDaysInc - 1]
        temp_output['Ky'] = M.Ky[1:M.NumDaysInc - 1]
        temp_output['Depletion_total'] = M.Depletion_total[1:M.NumDaysInc - 1]
        temp_output['Percent_depletion_total'] = M.Percent_depletion_total[1:M.NumDaysInc - 1]
        temp_output['Irrigation'] = M.Irrigation[1:M.NumDaysInc - 1]
        temp_output['P'] = M.Ps[1:M.NumDaysInc - 1]
        temp_output['Rainfall'] = M.rainfall[1:M.NumDaysInc - 1]
        temp_output['Rain_infilt'] = M.Rain_Infilt[1:M.NumDaysInc - 1]
        temp_output['Field_capacity_ave'] = M.FCave[1:M.NumDaysInc - 1]
        temp_output['Permanent_wilting_point_ave'] = M.PWPave[1:M.NumDaysInc - 1]
        temp_output['TAWsum'] = M.TAWsum[1:M.NumDaysInc - 1]
        temp_output['RAWsum'] = M.RAWsum[1:M.NumDaysInc - 1]
        temp_output['Available_water'] = M.AW[1:M.NumDaysInc - 1]
        temp_output['Available_water_depth_ave'] = M.AW[1:M.NumDaysInc - 1]
        temp_output['Volumetric_water_content_ave'] = M.VWCave[1:M.NumDaysInc - 1]
        temp_output['Total_depth_water'] = M.TDW[1:M.NumDaysInc - 1]
        temp_output['Watertableelevation'] = M.TDW[1:M.NumDaysInc - 1]
        temp_output['HeightEqu'] = M.Equilibrium_Max[1:M.NumDaysInc - 1]
        temp_output['NumberEqu'] = M.zu_Eq_Max[1:M.NumDaysInc - 1]
        temp_output['WaterstressKs'] = M.zu_Eq_Max[1:M.NumDaysInc - 1]
        temp_output['IrrigationSaltAdded'] = M.Irr_Salt_Added[1:M.NumDaysInc - 1]
        temp_output['WasteSaltAdded'] = M.Waste_Salt_Added[1:M.NumDaysInc - 1]
        temp_output['SeepageSaltLost'] = M.Seepage[1:M.NumDaysInc - 1]
        temp_output['SumOfFluxes'] = M.SumOfFluxes[1:M.NumDaysInc - 1]
        temp_output['TotalSalt'] = M.Total_salt[1:M.NumDaysInc - 1]
        temp_output['DifferenceInSalt'] = M.SaltDiff[1:M.NumDaysInc - 1]
        temp_output['SaltStress'] = M.Ks_salt[1:M.NumDaysInc - 1]
        temp_output['NitStress'] = M.Ks_nit[1:M.NumDaysInc - 1]
        temp_output['TotalIrrN'] = M.TotalIrrN[1:M.NumDaysInc - 1]
        temp_output['TotalDrainN'] = M.TotalDrainN[1:M.NumDaysInc - 1]
        temp_output['SumReactionsN'] = M.SumReactionsN[1:M.NumDaysInc - 1]
        temp_output['SumFluxReactionsN'] = M.SumFluxReactionsN[1:M.NumDaysInc - 1]
        temp_output['TotalMin'] = M.Total_Min[1:M.NumDaysInc - 1]
        temp_output['TotalDen'] = M.Total_Den[1:M.NumDaysInc - 1]
        temp_output['TotalFer'] = M.Total_Fer[1:M.NumDaysInc - 1]
        temp_output['TotalUpt'] = M.Total_Upt[1:M.NumDaysInc - 1]
        temp_output['UptakeReq'] = M.UptakeReq[1:M.NumDaysInc - 1]
        temp_output['TotalNit'] = M.Total_nit[1:M.NumDaysInc - 1]
        temp_output['TotalNitAccum'] = M.Total_nit_accum[1:M.NumDaysInc - 1]
        temp_output['NitAve'] = M.N_ave[1:M.NumDaysInc - 1]
        temp_output['NOptimalLow'] = M.N_optimal_low[1:M.NumDaysInc - 1]
        temp_output['NOptimalHigh'] = M.N_optimal_high[1:M.NumDaysInc - 1]
        temp_output['CumMin'] = M.Cum_Min[1:M.NumDaysInc - 1]
        temp_output['CumDen'] = M.Cum_Den[1:M.NumDaysInc - 1]
        temp_output['CumFer'] = M.Cum_Fer[1:M.NumDaysInc - 1]
        temp_output['CumUpt'] = M.Cum_Upt[1:M.NumDaysInc - 1]
        temp_output['CumIrrN'] = M.CumIrrN[1:M.NumDaysInc - 1]
        temp_output['CumDrnN'] = M.CumDrnN[1:M.NumDaysInc - 1]
        temp_output['CumChangeN'] = M.CumChangeN[1:M.NumDaysInc - 1]
        
        Number_layers_out = (M.NumDaysInc - 1) * (M.Num_layers + 1) + 1

        M.Create_output_layer_arrays(Number_layers_out, M.NumDaysInc, M.Num_layers)
